...
/* DB 연관 코드는 함수 2개
1. add_vertex-> 정점 추가, 새 ID 생성, DB에 새 정점 추가
2. delete_vertex->정점 삭제, 해당 ID를 가진 정점 삭제
- constructor는 계획 객체 생성자인데 App.js에서 ID 생성 후 짬통임, 계획 ID은 App.js에서 생성
*/

export default class DirectedGraph {
    constructor(id, vertices, first, height, width) {
      this.id=id;
      this.vertices = vertices || {}; // 정점들을 저장하는 객체, { id: Vertex객체 }
      this.height=height || 300;
      this.width=width || 965; //가로세로 기본값
      this.first=first || this.add_vertex(0, {goal:'첫 목표', prog:0, start:'', end:'',  x:20, y:this.height/2}).id; //첫번째 노드 ID
    }
  
    add_vertex(id, data) {  //정점 추가, 새 ID 생성
      const new_vertex = new Vertex(id, data);
      if (!(id in this.vertices)) {
        this.vertices[id] = new_vertex;
      }
      return new_vertex;  //반환도 해줌
    }
    ...
    append_next(vertex_id) {
      ...
      if (vertex.next.length&&vertex.data.x+135 >= this.vertices[vertex.next[0]].data.x){
        Object.entries(this.vertices).forEach(([key, value]) => { //분기한 같은 줄 같이 이동
          if (value.data.x > vertex.data.x) {
            value.data.x += 135;
          }});
      }
      this.add_vertex(newId, {goal:'목표', prog:0, start:'', end:'', x:vertex.data.x+135, y:vertex.data.y});
      ...
      for (const i of next) {
        ...
        if (this.vertices[i].data.x >= this.width)//캔버스 초과시
          this.width=this.vertices[i].data.x+135;
      }
      let maxX=0;
      Object.entries(this.vertices).forEach(([key, value]) => { //너비조정
        maxX=maxX > value.data.x ? maxX : value.data.x;
        });
      this.width=maxX+135 < this.width ? this.width : maxX+135;
    }
    ...
    branch(vertex_id) {
      ...
      if (prev.length === 1 && next.length === 1) { //0-0-0의 경우만 분기 가능
        ...
        this.add_vertex(newId, {goal:'목표', prog:0, start:'', end:'', x:vertex.data.x, y:vertex.data.y});
    ...
    delete_vertex(vertex_id) {  //정점 삭제
      if (vertex_id in this.vertices) {
        ...
        delete this.vertices[vertex_id];  //객체 삭제 코드
      }
    }
    ...
    execute(prev, next, vertex_id) {  //정점 제거 실행, 경우가 많아서 함수로 뺌
      ...
      Object.entries(this.vertices).forEach(([key, value]) => {
        minY=minY<value.data.y ? minY : value.data.y;
        maxX=maxX > value.data.x ? maxX : value.data.x;
        maxY=maxY > value.data.y ? maxY : value.data.y;
        if (value.data.x === vertexX) {
          count.push(value);  //같은 x 좌표의 노드(분기 유무) 확인
        }});
      console.log("111111111111111111111111111111", this.width, maxX)
      this.width=maxX-135 < this.width ? ((maxX < 965-135) ? 965 : maxX) : this.width; //너비 동적 조절
      this.width=this.width<965 ? 965 : this.width;
      Object.entries(this.vertices).forEach(([key, value]) => {
        if (maxY >= this.height&&!maxY < 300-45){  //노드 y좌표 동적 조절
          value.data.y -= (maxY-minY-this.height)/2;
        }
        if (count.length!==1&&count.includes(value)) {  //있으면 해당 x좌표 노드 재배열
          ...
          value.data.x -= 135;
        }else if (count.length===1&&value.prev.length&&value.next.length){ //삭제하고 그 열에 하나남았을때
          count[0].data.y=this.vertices[count[0].prev[0]].data.y;
        }
      });
      this.height=maxY-45 < this.height ? ((maxY < 300-45) ? 300 : maxY) : this.height; //높이 동적 조절
    }
